...
let rec print_term (t : term) : unit =
	match t with
	| TVar(index) -> Printf.printf "v%d" index
	| TDrf(t') ->
		print_string "drf(";
		print_term t';
		print_string ")"
...
let domain_extend (s1 : abstract_store) (s2 : abstract_store) : abstract_store =
	...
	OrderedAbstractLocationMap.fold (fun pi theta acc ->
		let pi_bindings = OrderedAbstractValueSet.fold (fun (pi_i, _) acc ->
			if OrderedAbstractLocationSet.mem pi dom_s1
			then
				let s1_pi = OrderedAbstractLocationMap.find pi s1 in
			...
			else OrderedAbstractValueSet.add (pi_i, cFalsePair) acc) theta OrderedAbstractValueSet.empty in
		OrderedAbstractLocationMap.add pi pi_bindings acc) s2 s1
...
let get_location_variables_at_all_levels (v : varinfo) (index : int) : location_variable_set =
	let rec helper t acc s = 
		...
		| TInt(_, _) -> OrderedLocationVariableSet.add acc s
		| _ -> assert false; s
		) in
...
let compose_heap (s_f : abstract_store) 
	...
	OrderedAbstractLocationMap.fold (fun pi_i theta_i acc ->
		compose_partial_heap pi_i theta_i s i rho
	) s_f s
...
let rec transform_instruction (instruction : instr)
	...
	| Call(lvalue, func_exp, argument_list, loc) ->
		...
		| "malloc" ->
			begin
			match lvalue with
			| None -> s, phi (* This should not happen *)
...
and transform_statement (statement : stmt) 
	...
	| If(_, block1, block2, _) ->
		...
		(join_abstract_store s1 s1), (bracketing_constraint_conjunction phi1 phi2)
	| _ -> s, phi
...
let get_function_by_name (function_name : string) (cur_file : file) : fundec =
  	...
  	match f with
  	| None -> Printf.printf "Error: Function not found by name\n"; exit 1; dummyFunDec
    | Some(f') -> f'

let do_heap_analysis (cur_file : file) : unit =
	let partitions =[["main"]; ["foo"]] (*get_call_graph_partitions_in_rev_topo_order cur_file*) in
